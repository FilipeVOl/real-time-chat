...
const Chat = () => {
  ...
  return (
    <div className='w-auto'>
      <div className='border-2 bg-[#accffe] divide-blue-400'>
      <div className='flex flex-col items-center p-4 border-b-2 border-blue-400 mb-8'>
        ...
        {profilesArray.map((profile) => (
          <div key={profile.id} className='flex flex-col gap-4 cursor-pointer border-b-2 hover:border-2 rounded-lg'> {/* Added key prop */}
          <div className='flex flex-row gap-4 items-center p-4 hover:scale-105'>
            ...
            <img className="w-4" src={profile.status === "Available" ? Online : Offline} alt="status" />
            <h1 className='text-xl font-semibold'>{profile.status}</h1> {/* Changed line */}
            </div>
            </div>
            </div>
          </div>
